...
//-----------------------------------------------------------------------------
bool RenderObject::Collisions(const Vector3& point, Vector3& outPoint)
{
	Vector3 pos = point;
	pos.y += 0.5f;
	...
	if ((boxHitInfo.hit) && (boxHitInfo.distance < 1))
	{
		...
		RayCollision meshHitInfo = GetRayCollisionMesh(ray, m_model.meshes[0], matTransform);
		if (meshHitInfo.hit)
		{
			outPoint = meshHitInfo.point;
			return true;
		}
	}
	return false;
